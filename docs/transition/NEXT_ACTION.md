...
Last verified commit: `b1482ad` (`Count only Edge qualifications for Typebot removal`), with focused decommission-readiness PostgreSQL tests, `npm ci`, tracked artifact scan, TypeScript lint, full Vitest suite, production build, `dist/tests` exclusion check, moderate audit, smoke test, and integration smoke all passing. The first `npm audit --audit-level=moderate` attempt failed on transient DNS resolution for `registry.npmjs.org`; a retry succeeded and found 0 vulnerabilities.
