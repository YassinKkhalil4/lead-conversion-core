...
Last verified commit: `4b60e30` (`Require published active config for Typebot removal`), with focused decommission-readiness PostgreSQL tests, `npm ci`, tracked artifact scan, TypeScript lint, full Vitest suite, production build, `dist/tests` exclusion check, moderate audit, smoke test, and integration smoke all passing.
