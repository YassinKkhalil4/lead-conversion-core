...
Last verified commit: `f12d590` (`Require website and Facebook lead stability`), with focused decommission-readiness PostgreSQL tests, `npm ci`, tracked artifact scan, TypeScript lint, full Vitest suite, production build, `dist/tests` exclusion check, moderate audit, smoke test, and integration smoke all passing.
