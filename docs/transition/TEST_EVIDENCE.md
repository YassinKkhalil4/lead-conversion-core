...
Result: passed from commit `3cba1c3`. `npm ci` installed 118 packages and found 0 vulnerabilities; tracked artifact scan passed; TypeScript lint passed; Vitest ran 15 files and 128 tests; build passed; audit found 0 vulnerabilities; smoke returned `ok=true`; integration smoke returned `ok=true` with 12 stop conditions checked.
...
Result: passed. Focused app/deployment-script tests ran 4 tests, including enabled direct-lead deployment probes that sent only `eventId`/`clientKey` for website and `leadgen_id`/`clientKey` for Facebook, expected `invalid_lead_payload` responses, and verified no phone/name/field-data business lead payload was sent by the verifier. This historical route contract was later superseded by DEC-039 and the current durable-receipt verifier checks.
...
Command: `npx vitest run tests/runtime.integration.test.ts -t "daily report"`

Result: passed. Focused PostgreSQL run executed 4 daily-report tests and skipped 59 by filter, including next daily report materialization after successful send with a Europe/Berlin DST boundary.
...
Result: passed. Focused PostgreSQL run executed 3 decommission-readiness tests and skipped 61 by filter, including a regression case proving ignored direct-ingress deployment probes do not count as stability evidence.
...
Result: passed. Focused PostgreSQL run executed 2 cutover-readiness tests and skipped 62 by filter, including a due scheduled-job backlog failure while a future scheduled job did not increase the pending count.
...
Result: passed. Focused PostgreSQL run executed 7 tests and skipped 69 by filter, covering decommission blocker reporting, ignored probe exclusion, status-callback exclusion, disabled current direct ingress, missing runtime worker heartbeat, missing direct-lead processor metadata, and a passing exit-evidence report with a matching direct-ingress runtime heartbeat.

Command: `npm ci && npm run artifacts:scan && npm run lint && npm test && npm run build && test ! -d dist/tests && npm audit --audit-level=moderate && npm run test:smoke && npm run test:integration`

Result: passed. `npm ci` installed 118 packages and found 0 vulnerabilities; tracked artifact scan passed; TypeScript lint passed; Vitest ran 16 files and 158 tests; build passed; `dist/tests` was absent; audit found 0 vulnerabilities; smoke returned `ok=true`; integration smoke returned `ok=true` with 12 stop conditions checked.

## 2026-08-01 Decommission Route-Family Stability Evidence

Command: `npx vitest run tests/runtime.integration.test.ts -t "decommission"`

Result: passed. Focused PostgreSQL run executed 8 tests and skipped 69 by filter, including a regression case proving Meta-only aged processed direct-ingress evidence does not satisfy direct-ingress stability while direct lead ingress is also enabled.

Command: `npm ci && npm run artifacts:scan && npm run lint && npm test && npm run build && test ! -d dist/tests && npm audit --audit-level=moderate && npm run test:smoke && npm run test:integration`

Result: passed. `npm ci` installed 118 packages and found 0 vulnerabilities; tracked artifact scan passed; TypeScript lint passed; Vitest ran 16 files and 159 tests; build passed; `dist/tests` was absent; audit found 0 vulnerabilities; smoke returned `ok=true`; integration smoke returned `ok=true` with 12 stop conditions checked.

## 2026-08-01 N8n Inbox Dead Letters Block Decommission

Command: `npx vitest run tests/runtime.integration.test.ts -t "decommission"`

Result: passed. Focused PostgreSQL run executed 9 tests and skipped 69 by filter, including an aged n8n `dead_lettered` inbox event that fails `no_unresolved_n8n_inbox` while `no_recent_n8n_compat_usage` passes.
...
Result: passed. Focused PostgreSQL run executed 10 tests and skipped 69 by filter, including an aged processed n8n salesperson command inbox receipt with a rejected `app.salesperson_commands` row. The report passed `no_unresolved_n8n_inbox`, `no_recent_n8n_compat_usage`, `direct_ingress_stable`, and `direct_ingress_worker_operational`, while failing `no_rejected_n8n_salesperson_commands`.
...
Result: passed. Focused PostgreSQL run executed 12 tests and skipped 69 by filter, including a cancelled `airtable.project_lead_visibility` outbox command that failed `airtable_projection_outbox_stable` while `airtable_projection_only_verified`, `airtable_reconciliation_stable`, `final_airtable_export_complete`, and `owner_approved_airtable_decommission` passed.
...
Result: passed. Focused route/deployment-script run executed 6 tests. The disabled direct-ingress test proved direct Meta challenge and POST paths both return HTTP 503 when disabled, while n8n compatibility remains separately gated and available.
...
Result: passed. TypeScript accepted the recent legacy activity decommission metric, checks, and regression fixture.

Command: `npx vitest run tests/runtime.integration.test.ts -t "decommission"`

Result: passed. Focused PostgreSQL decommission run executed 16 tests and skipped 71 by filter, including an old terminal legacy-owned conversation with recent activity that fails both n8n and Typebot readiness.

Command: `npm test`

Result: passed. Vitest ran 17 files and 176 tests.
...
Result: passed. TypeScript accepted the cutover readiness environment-provider injection and direct-ingress target check.

Command: `npx vitest run tests/runtime.integration.test.ts -t "cutover readiness"`

Result: passed. Focused PostgreSQL cutover-readiness coverage ran 7 tests and skipped 78 by filter, including a fallback-only fixture where `N8N_COMPAT_ROUTES_ENABLED=true` and no direct ingress route family is selected.
...
Result: passed. TypeScript accepted the per-source direct lead stability metrics and readiness details.

Command: `npx vitest run tests/runtime.integration.test.ts -t "n8n semantic scheduled|website and Facebook|decommission"`

Result: passed. Focused PostgreSQL readiness coverage ran 20 tests and skipped 71 by filter, including website-only direct lead stability failing while both website and Facebook evidence satisfy the positive fixture.
...
Result: passed. Vitest ran 17 files and 180 tests.
...
Result: passed before full gate execution. Focused parser coverage ran 2 tests and skipped 8 by filter, including unsafe cutover and decommission thresholds above `Number.MAX_SAFE_INTEGER`.
...
Command: `npx vitest run tests/ingress-gating.test.ts tests/shell-scripts.test.ts`

Result: passed before full gate execution. Focused coverage ran 2 files and 18 tests, proving deployment verifier durable probes honor explicit run IDs and shell scripts avoid second-only event IDs.
...
Command: `npx vitest run tests/shell-scripts.test.ts tests/ingress-gating.test.ts`

Result: passed before full gate execution. Focused coverage ran 2 files and 19 tests, including parser proof that command-substitution text remains literal and no marker file is created.

Command: `bash -n scripts/verify-deployment.sh`

Result: passed. Deployment verifier shell syntax is valid.

Command: `bash -n scripts/shadow-sequence.sh`

Result: passed. Shadow sequence shell syntax is valid.
...
Result: passed. Vitest ran 17 files and 183 tests.
...
Result: passed before full gate execution. Focused shell-script coverage ran 12 tests, including duplicate `DIRECT_META_WEBHOOK_ENABLED` rejection.
...
Result: passed. Vitest ran 17 files and 184 tests.
...
Result: passed before full gate execution. Focused coverage ran 2 files and 20 tests, including static checks that both operator scripts chmod parsed env assignment temp files private.
...
Result: passed before full gate execution. Focused shell-script coverage ran 13 tests, including escaped NUL and escaped newline value rejection.
...
Result: passed. Vitest ran 17 files and 185 tests.
...
Result: passed before full gate execution. Focused shell/operator parser coverage ran 17 tests, including strict calendar reconciliation command/flag/UUID/limit/text validation.
...
Result: passed. Vitest ran 17 files and 189 tests.
...
Command: `npx vitest run tests/shell-scripts.test.ts tests/config-versioning.test.ts`

Result: passed before full gate execution. Focused shell/config parser and config-versioning coverage ran 2 files and 22 tests, including strict configuration command/source/actor/version argument validation.
...
Result: passed. Vitest ran 17 files and 190 tests.
...
Command: `npx vitest run tests/runtime.integration.test.ts -t "seeds configuration|publishes immutable versioned configuration|activates and rolls back"`

Result: passed before the full gate. Focused PostgreSQL configuration coverage ran 1 file, 3 tests, and 90 skipped by filter.
...
Result: passed. Vitest ran 17 files and 196 tests.
...
Command: `npx vitest run tests/shell-scripts.test.ts tests/runtime.integration.test.ts -t "seed CLI|seeds configuration"`

Result: passed before the full gate. Focused seed parser and PostgreSQL seed coverage ran 2 files, 2 tests, and 111 skipped by filter.
...
Result: passed. Vitest ran 17 files and 197 tests.
...
Command: `npx vitest run tests/migrate.test.ts tests/shell-scripts.test.ts`

Result: passed before the full gate. Focused migrator checksum/parser and shell-script coverage ran 2 files and 25 tests.
...
Result: passed. Vitest ran 17 files and 198 tests.
...
Result: passed before the full gate. Focused shell-script coverage ran 1 file and 21 tests, including executable backup/restore argument rejection before secret-bearing env reads.
...
Result: passed. Vitest ran 17 files and 199 tests.
...
Result: passed before the full gate. Focused coverage ran 2 files and 32 tests, including deployment verifier duplicate/empty/control/unknown argument rejection before env loading, env-only shadow/dump script argument rejection before env/Docker inputs, and existing direct-ingress verifier behavior.
...
Result: passed. Vitest ran 17 files and 201 tests.
...
Result: passed before the full gate. Focused PostgreSQL coverage ran 1 file and 94 tests, including outbox idempotency key collision rejection and scheduled-job semantic key collision rejection.
...
Result: passed. Vitest ran 17 files and 203 tests.
...
Result: passed before the full gate. Focused PostgreSQL coverage ran 1 file and 96 tests, including changed-slot appointment booking idempotency collision rejection.
...
Result: passed. Vitest ran 17 files and 205 tests.
...
Result: passed before the full gate. Focused PostgreSQL runtime coverage ran 1 file and 98 tests, including legacy `edge_outbox` changed-semantics idempotency collision rejection.
...
Result: passed. Vitest ran 17 files and 207 tests.
