...
Deliverables: lockfile, fixed compiled paths, runtime-only production build output, one-shot migrator with closed no-argument CLI parsing, migration checksums/advisory lock, readiness, operational worker heartbeat checks, backup/restore scripts with private libpq service-file connection handling, backup output locking/refusal for timestamped encrypted dump/checksum collisions, encrypted dump checksum verification before restore, PostgreSQL 16 dump inspect/restore-smoke wrappers, environment template, enabled-integration credential validation, generated env rendering that avoids secret-bearing process arguments, container topology with API/worker startup free of migrations and configuration seeding.

Dependencies: Docker daemon for image run and Docker-based dump inspection.

Verification gates: serial npm gate, disposable PostgreSQL migration/idempotency/checksum/concurrency, migration CLI argument parser coverage, backup/restore, readiness, heartbeat, environment contract, static Compose config, shell syntax checks.
...
Deliverables: import/validate/diff/publish/rollback/active CLI with fail-closed argument parsing, seed bootstrap through versioned configuration authority with closed no-argument CLI parsing, immutable published config, conversation pins to immutable configuration version IDs with legacy `config_version` compatibility, disabled-by-default legacy config import and Airtable sync compatibility gates, and Airtable export compilation for Questions, Question Options, and Conversation Messages.

Dependencies: complete config source for final parity.

Verification gates: invalid config rejection, versioned seed idempotency, seed CLI argument parser coverage, seed/export parity compile, rollback, conversation pinning, command-specific configuration CLI parser coverage, and disabled legacy config mutation route/CLI coverage.
...
Completed locally: direct Meta webhook route is disabled by default unless `DIRECT_META_WEBHOOK_ENABLED=true`; direct website/Facebook lead ingress is disabled by default unless `DIRECT_LEAD_INGRESS_ENABLED=true`; disabled direct-ingress checks return unavailable before provider token, signature, or edge shared-secret validation; direct route flags and n8n compatibility routes fail startup validation without their required runtime worker processing flags; direct Meta signed unsupported webhooks are durably receipted and then ignored by the runtime worker; direct lead routes authenticate only when enabled, durably receipt, deduplicate, and acknowledge without running lead-intake business logic inside the HTTP request; runtime inbox workers claim only configured provider/event-type subsets and process website/Facebook lead receipts into `LeadIntakeService`; legacy synchronous `/v1/turn` active compatibility is disabled by default unless `ACTIVE_TURN_COMPAT_ENABLED=true`; n8n compatibility routes remain independently gated by `N8N_COMPAT_ROUTES_ENABLED` and callback routes durably receipt before client relationship resolution; n8n compatibility now wires the shared WhatsApp/status/salesperson inbox processor independently of direct Meta webhook processing so fallback callback rows remain processable while direct Meta is disabled; `WORKER_KIND` is a closed deployment role (`outbox` or `runtime`) so typoed worker roles fail startup validation before selecting the wrong worker loop; local app-injection tests prove disabled direct routes and legacy active-turn compatibility return 503 without provider calls while n8n compatibility remains available when explicitly enabled with the runtime worker; `npm run cutover:readiness` provides a read-only PostgreSQL-backed readiness report for direct-route flags, requires at least one direct-ingress route family before passing, requires direct-inbox processor metadata including `whatsapp.webhook_ignored` for enabled direct Meta, requires direct lead processor metadata for enabled lead ingress, requires n8n compatibility inbox processor metadata when fallback routes are enabled, and reports legacy active-turn compatibility state, n8n compatibility, pending/oldest inbox, outbox, due scheduled-job work, delivery-unknown counts, terminal outbox failure counts, dead-letter counts, and runtime worker heartbeat age plus operational metadata; future scheduled jobs are reported separately from due work and do not block cutover readiness; disabled runtime worker heartbeats or heartbeats missing required direct/n8n inbox provider-event metadata do not satisfy cutover readiness when the corresponding route family is enabled; cutover/decommission readiness CLIs reject unknown or malformed operator arguments, and threshold arguments accept only canonical base-10 non-negative safe-integer strings, before querying PostgreSQL; `docs/transition/DIRECT_INGRESS_PLAN.md` documents staging and production canary route shapes, required flags, rollback, and decommission hold; `scripts/verify-deployment.sh` accepts `--base-url`, `--env-file`, direct ingress check flags, n8n compatibility check flags, and skip flags so staging can verify direct Meta challenge and POST enabled/disabled behavior, signed non-customer Meta webhook receipt, unsigned Meta webhook rejection, direct website/Facebook lead ingress enabled/disabled behavior, and n8n compatibility fallback enabled/disabled behavior without real customer messages, while loading one-line env values through a parser instead of shell `source`, rejecting duplicate env keys and decoded NUL/newline values, chmodding parsed env assignment temp files private, keeping those values shell-local, and passing sensitive curl inputs through private temporary files; disabled direct-route verification does not require Meta credentials or `EDGE_SHARED_SECRET` when shadow verification is skipped; disabled n8n compatibility verification still requires `EDGE_INTERNAL_SECRET` because it verifies an internal authenticated route; `scripts/verify-deployment.sh` and `scripts/shadow-sequence.sh` use run-scoped durable event IDs to avoid probe collisions during quick reruns; `scripts/shadow-sequence.sh` also loads `.env` without shell execution, duplicate-key overrides, or decoded NUL/newline values, chmods parsed env assignment temp files private, keeps env values shell-local, and passes `EDGE_SHARED_SECRET` through a private temporary header file; the enabled direct-lead and enabled n8n fallback checks expect durable receipt acknowledgement from non-business probes instead of creating authoritative leads or customer messages; `npm run decommission:readiness` provides a read-only PostgreSQL-backed exit report for n8n, Typebot, and Airtable projection retirement, including current direct-ingress runtime authority, fresh matching runtime-worker heartbeat metadata, legacy active-turn compatibility state, n8n compatibility route disablement, legacy conversation activity, recent legacy-owned conversation activity, active legacy-only config snapshot detection, published active configuration pointer checks, Edge-owned qualification volume checks, n8n scheduled authority checks across durable job semantic identity fields, completed-at direct business-ingress stability evidence matched to every currently enabled direct route family, per-source website and Facebook direct-lead stability checks, parked or dead-lettered legacy edge outbox work, unresolved or dead-lettered n8n compatibility work, rejected n8n salesperson command outcomes, Airtable projection/reconciliation state including cancelled projection commands, missing required reconciliation check evidence, and non-pass reconciliation results, migrated content/config evidence, final export flags, and explicit owner approval flags. Ignored deployment probes and provider delivery-status callbacks do not count as decommission stability evidence.
