...
7. Run `npm run cutover:readiness -- --max-pending-inbox=0 --max-pending-outbox=0 --max-pending-scheduled-jobs=0 --max-queue-age-seconds=300` and review all `fail` checks before changing external routes. The report fails if no direct-ingress route family is enabled; n8n fallback availability alone is not cutover readiness.
