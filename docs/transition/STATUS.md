...
| MP-06 Versioned configuration | implementing | local PostgreSQL integration and invalid fixture tests | Immutable published configuration versions, active pointers, validation/diff/publish/active/rollback service and CLI foundation added while preserving legacy `edge_config_snapshots` compatibility. Runtime config reads prefer `configuration.active_versions` with legacy fallback; invalid empty-question configs are rejected; legacy conversations now pin the immutable `configuration.versions` UUID alongside `config_version`. |
