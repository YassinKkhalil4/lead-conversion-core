...
| Verification | locally_verified | `npm ci`, `npm run lint`, `npm test` (19 files, 183 tests), `npm run build`, `npm audit --audit-level=moderate`, `npm run test:smoke`, focused metrics/calendar/config tests, and a disposable PostgreSQL `npm run seed` check all passed locally. |
