...
| MP-06 Versioned configuration | implementing | local PostgreSQL integration tested for publish, active pointer, rollback, and repository reads | Immutable published configuration versions, active pointers, validation/diff/publish/active/rollback service and CLI foundation added while preserving legacy `edge_config_snapshots` compatibility. Runtime config reads prefer `configuration.active_versions` with legacy fallback. |
