...
| MP-07 Lead intake and CRM | implementing | local PostgreSQL/API integration tested for internal intake | Authenticated internal lead intake now validates source payloads, upserts contacts/leads idempotently, matches projects without inventing relationships, records audit metadata, suppresses first contact for opted-out contacts, and enqueues first-contact WhatsApp outbox commands without provider calls. Source-specific website/Facebook adapters and Airtable projection-only hook remain. |
