...
| MP-05 WhatsApp messaging platform | implementing | local contract fixtures and PostgreSQL/API integration tests | Provider-neutral messaging port, disabled Meta WhatsApp adapter, durable outbox dispatcher mapping, internal WhatsApp message request API/service, template allow-list validation, 24-hour session-window policy, and Meta status webhook durable inbox ingestion/processing added. Current direct Meta send remains in active turn service until compatibility and cutover gates are implemented. |
