...
| Verification | locally_verified | `npm ci`, `npm run lint`, `npm test` (17 files, 142 tests), `npm run build`, `npm audit --audit-level=moderate`, `npm run test:smoke`, focused metrics/calendar/config tests, and a disposable PostgreSQL `npm run seed` check all passed locally. |
