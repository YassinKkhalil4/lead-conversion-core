...
5. Run `npm run cutover:readiness -- --max-pending-inbox=0 --max-pending-outbox=0 --max-pending-scheduled-jobs=0 --max-queue-age-seconds=300`; it must show `direct_ingress_target_selected=pass`, so n8n fallback alone is not enough.
...
7. Use `scripts/verify-deployment.sh --base-url=<staging-url> --check-direct-meta --check-direct-lead --check-n8n-compat` with explicit `--expect-direct-meta=<enabled|disabled>`, `--expect-direct-lead=<enabled|disabled>`, and `--expect-n8n-compat=<enabled|disabled>` to prove the intended direct-ingress and fallback state before changing external routes. The verifier parses one-line env values without `source` or shell command execution; duplicate env keys and decoded NUL/newline values fail closed. The enabled direct-Meta check verifies the challenge route, sends a signed non-customer webhook probe that should be durably acknowledged through the ignored-webhook path, and sends an unsigned POST that must be rejected. The enabled direct-lead check probes both website and Facebook routes with deliberately incomplete payloads and expects durable receipt acknowledgement; it must not create an authoritative lead or outbound command inside the webhook request. The enabled n8n compatibility check uses an authenticated, non-customer inbound fallback probe and expects durable receipt acknowledgement. Probe event IDs are run-scoped by default so quick verifier reruns do not collide with durable inbox deduplication; set `VERIFY_DEPLOYMENT_RUN_ID` only when intentionally reproducing one verifier run. Disabled direct-ingress checks require no Meta token, app secret, or edge shared secret when shadow verification is skipped; they prove the routes return unavailable before authentication or durable receipt. Disabled n8n compatibility verification still requires `EDGE_INTERNAL_SECRET` because `/compat/n8n/**` is an internal authenticated route.
...
5. Monitor `npm run cutover:readiness` output, provider callbacks, inbox/outbox age, terminal outbox failures, delivery-unknown counts, dead letters, and runtime worker heartbeat.
6. Stop expanding canary traffic if any rollback threshold in `docs/owner-actions/07-production-cutover.md` is met.
7. Use `npm run decommission:readiness` only after the canary window has produced enough direct-ingress evidence measured from successful processing completion; do not treat a passing report as removal approval.
