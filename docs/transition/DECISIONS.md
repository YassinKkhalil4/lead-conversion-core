...
Decision: Operator shell scripts that need `.env` values load assignments through `scripts/ops/read-env-file.py`, exporting parsed one-line `KEY=value` records without `source`, `eval`, shell command execution, duplicate-key override behavior, or decoded NUL/newline values. Temporary files that hold parsed env assignments are explicitly chmodded private before values are written.
