...
## 2026-08-16 Dashboard Reply Status Codes And Diagnosable Request Failures

- Defect found and fixed: every failed request logged `"error": {}`. `Error.message` and `Error.stack` are non-enumerable, and pino had a serializer registered only for `req`, so an `Error` logged under the `error` key serialised to an empty object. Added `serializeErrorForLog` and registered it for both the `error` and `err` keys.
- Implementation slice: the error serializer records `type`, `message`, `stack`, `statusCode`, the `cause` chain to a depth of three, and the PostgreSQL fields that identify a failure (`code`, `severity`, `constraint`, `table`, `schema`, `column`, `routine`). It deliberately omits `detail`, `hint`, `where`, and `internalQuery`, which echo the offending row values and are lead PII.
- Defect found and fixed: `POST /api/leads/:id/reply` answered `500` for caller errors. `MessageRequestService` signals send-policy refusals with plain `Error`s, and the dashboard error handler maps anything without a `statusCode` to `500`, so an unapproved template returned `500` instead of `400`.
- Defect found and fixed: the same mapping made a time-of-check/time-of-use race in the reply path escape as an opaque `500`. The 24-hour session window is evaluated once in `DashboardLeadActionService.reply` and again inside `MessageRequestService.validatePolicy`; a reply sent close to the boundary can cross it between the two checks, which is why the failure was load-dependent and passed on re-run and in isolation. Send-policy refusals are now translated to `409 session_window_closed` and `400 template_not_approved`, so the race degrades to the correct, actionable status instead of a server fault. `MessageRequestService` remains the fail-closed authority and is unchanged.
- Defect found and fixed in the test harness: `tests/dashboard-api.integration.test.ts` drew its disposable-cluster port from `57_500 + random(1000)`, the identical range already used by `tests/health-readiness.integration.test.ts`. A collision does not fail loudly: a deliberate two-cluster experiment confirmed the second `pg_ctl start` fails to bind while `createdb` still succeeds against the cluster that won the bind, so the losing file would silently operate inside the winner's cluster. The dashboard file moved to a dedicated `60_500` range, and the per-file range allocation is now documented in the test.
- Decision: send-policy refusals are caller errors, not server faults. The dashboard translates them at its boundary rather than weakening the shared service's fail-closed policy.
- Verification note: the reported intermittent failure did not reproduce in twenty consecutive full-suite runs. The cause was established by reading the reply path and by the deliberate port-collision experiment, not by observing the flake.
- Verification: `npm run lint` passed, `npm test` passed three consecutive times with 19 files and 187 tests, and `npm run build` passed with no `dist/tests`.
